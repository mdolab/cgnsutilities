...
def main():
    ...
    p_refine.add_argument('--axes', nargs='+', help="Refine mesh only along specified axis or axes ( default is all three axes): 'i', 'j', 'k'\
                                                    ", default=['i','j', 'k'])
    ...
    p_extract_spec = subparsers.add_parser('extractSpecified',
                                        help='Extract a surface from a specified set of layers in a cgns block')
    ...
    p_connect.add_argument('--connectSelf', help='only check for connection on-block (periodic type)',
                        action='store_true', dest='connectSelf', default=False)
    ...
    {iLow, iHigh etc}, newFamilyName <genI_start genI_end genJ_start genJ_end> \
    genI_start means generalized I start etc. That is once the face is identified, these \
    are the two remaining indices. If it is a I face, they are (j,k), J face they are (i, k) \
    and for a K face (i, j). These are optional and if given must be in 1 based ordering. Also \
    -1 is acceptable for the end index.'
                    )
    ...
    p_fam = subparsers.add_parser(
        'familysubface', help='Overwrite the family information on a subface \n \
    ...
    p_fam.add_argument('familyFile', help='file containing data for the new \
    ...
    p_rem = subparsers.add_parser(
        'removebc', help='Remove all BC')
    p_rem.add_argument('gridFile', help='Name of input CGNS file')
    p_rem.add_argument('outFile', nargs='?', default=None,
                    help='Optional output file')
    ...
    BCSetType     - the dataset bc type. This is in most cases the same type as the BCType specified \n\
    DirNeuArr     - can have one of two options: Dirichlet or Neumann \n\
    DataArrNameN  - name of first property specified. This can be a range of things. Refer to ICEM or SUMAD supported BC properties \n\
    ...
    p3dtoc = subparsers.add_parser(
        'plot3d2cgns',  help='Convert a multiblock, unformatted fortran, big-endian, multiblock plot3d file to a plain \
    ...
    p_sym = subparsers.add_parser(
        'symmzero', help='Hard-zero any nodes on symmetry plane BCs.')
    ...
    p_symnobc = subparsers.add_parser(
        'symmzeronobc', help='Hard-zero any nodes within a given tolerance of the symmetry plane. BCs are not taken into account.')
    p_symnobc.add_argument('gridFile', help='Name of input CGNS file')
    p_symnobc.add_argument('sym', help='Normal for possible symmetry plane.',
                        choices=['x', 'y', 'z'])
    p_symnobc.add_argument('--tol', help="Distance tolerance to bring nodes to symmetry plane.\n\
                                        Default is 1.0e-5.",
                        type=float, default=1.0e-5)
    p_symnobc.add_argument('outFile', nargs='?', default=None,
                        help='Optional output file')
    ...
    p_rm = subparsers.add_parser(
        'remove', help='Remove blocks from a cgns file. The user should ensure that the final mesh is still valid in terms of boundary conditions and connectivities.')
    p_rm.add_argument('gridFile', help='Name of input CGNS file')
    p_rm.add_argument('blockIDs', metavar='files', type=int, nargs='+', help='IDs (integers) of the blocks that will be removed.\n\
    ...
    p_expkmin = subparsers.add_parser(
        'explodeByZoneName', help='Take one multiblock cgns file and explode it into multiple multiblock \n\
    ...
    p_rot = subparsers.add_parser(
        'rotate', help='Rotate a grid around a given direction.')
    ...
    p_rot.add_argument('outFile', nargs='?', default=None,
                    help='Optional output file')
    ...
    p_obc.add_argument('sym', help='Normal for possible symmetry plane.',
                    choices=['x', 'y', 'z', 'none'])
    p_obc.add_argument('--connectSelf', help='only check for connection on-block (periodic type)',
                    action='store_true', dest='connectSelf', default=False)
    ...
    p_fbc = subparsers.add_parser(
        'fillOpenBCs', help='Adds a given BC to the faces that are not face-matched and \
        also that do not have any previously-assigned BCs.')
    p_fbc.add_argument('gridFile', help='Name of input CGNS file')
    p_fbc.add_argument('bocoType', help='Boundary condition type. Supported types are: \
    ...
    p_fbc.add_argument('outFile', nargs='?', default=None,
                    help='Optional output file')

    # ------------ Options for 'extractConv' mode  --------------------
    p_conv = subparsers.add_parser(
        'extractConv', help='Reads the convergence history node of a CGNS file and saves the \
    ...
    p_conv.add_argument('outFile', nargs='?', default=None, help='The convergence data will be saved to this filename. \
    ...
    p_p2D = subparsers.add_parser(
        'extrude', help='Takes a true 2D mesh (planar) and extrude it in one direction to make \
    ...
    p_p2D.add_argument('gridFile', help='Name of input CGNS file. Note that the planar grid should \
        not have symmetry BCs set on the plane.')
    p_p2D.add_argument('direction', help='Direction which to extrude the grid',
                    choices=['x', 'y', 'z'])
    p_p2D.add_argument('outFile', nargs='?', default=None,
                    help='Optional output CGNS file')


    # ------------ Options for 'revolve' mode
    p_p2R = subparsers.add_parser(
        'revolve', help='Takes a true 2D mesh (planar) and reloves it about specified axis to make \
    ...
    p_p2R.add_argument('gridFile', help='Name of input CGNS file. Note that the planar grid should \
        not have symmetry BCs set on the plane.')
    p_p2R.add_argument('normalDirection', help='This is the direction in which the plane normal points in. \
                        Example: If supplied data is in xz plane, the normal points in y',
                    choices=['x', 'y', 'z'])
    p_p2R.add_argument('axis', help='Axis which to rotate about \
                        Example: If supplied data is in xz plane, you would give either x or z',
                    choices=['x', 'y', 'z'])
    p_p2R.add_argument('startAngle', type=float, help='Rotation starting angle given in degrees. Typically this is a small quantity.')
    p_p2R.add_argument('endAngle', type=float, help='Rotation ending angle given in degrees. Typically this is a small quantity.')
    p_p2R.add_argument('nThetas', type=int, help='number of angular locations to put points', default=2)
    p_p2R.add_argument('outFile', nargs='?', default=None,
                    help='Optional output CGNS file')
    ...
    p_blockSizes = subparsers.add_parser('blockSizes',
                                        help='Print the sizes of each block in the mesh')
    ...
    if args.mode == 'explicitCart':
        ...
        simpleCart(xMin, xMax, args.dh, args.hExtra, args.nExtra,
                args.sym, args.mgcycle, args.outFile)
    ...
    if args.mode == 'extractSpecified':
        curGrid.extractSpecifiedSurface(args.surfFile,args.blockID,args.imin,args.imax,args.jmin,args.jmax,args.kmin,args.kmax)
        sys.exit(0)
    ...
    if args.mode == 'flip':
        ...
        if args.outType == 'pickle':
            ...
            write_tecplot_file(outFile,'Convergence',
                            ['Iteration']+curGrid.convArray.keys(),data)
