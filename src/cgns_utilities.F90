...
contains
    ...
    subroutine getBCDataArrayInfo(cg, iBlock, iBC, iBCDataSet, iDataArr, &
                                  bcTypeFlag, dataArrayName, dataType, nDataDimensions, &
                                  dataDimensionVector)
        ...
        ! Input/Output
        integer, intent(in) :: cg, iBlock, iBC, iBCDataSet, iDataArr, bcTypeFlag
        ...
        base = 1
        call cg_goto_f(cg, base, ier, 'Zone_t', iBlock, 'ZoneBC_t', 1, 'BC_t', &
                       iBC, 'BCDataSet_t', iBCDataSet, 'BCData_t', bcTypeFlag, 'end')
        if (ier .eq. CG_ERROR) call cg_error_exit_f
    ...
    subroutine getBCDataArray(cg, iBlock, iBC, iBCDataSet, iDataArr, bcTypeFlag, dataArr, nDataArr)
        ! This subroutine retrieves and returns BC dataset array

        implicit none

        ! Input/Output
        integer, intent(in) :: cg, iBlock, iBC, iBCDataSet, iDataArr, bcTypeFlag, nDataArr
    ...
    subroutine writeBCData(cg, iBlock, iBC, iDataSet, bcTypeFlag, writeBCDataHeader, &
        ...
        ! Input/Output
        integer, intent(in) :: cg, iBlock, iBC, iDataSet, bcTypeFlag
        ...
        if (writeBCDataHeader) then
            call cg_bcdata_write_f(cg, base, iBlock, iBC, iDataSet, bcTypeFlag, ier)
            if (ier .eq. CG_ERROR) call cg_error_exit_f
        end if

        call cg_goto_f(cg, base, ier, 'Zone_t', iBlock, "ZoneBC_t", 1, &
                       "BC_t", iBC, "BCDataSet_t", idataSet, "BCData_t", bcTypeFlag, "end")
        if (ier .eq. CG_ERROR) call cg_error_exit_f
