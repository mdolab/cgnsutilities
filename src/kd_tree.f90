...
!  end function compare_priority
    ...
    subroutine heapify(a, i_in)
        ...
        real(kind=8) :: pri_i, pri_l, pri_r, pri_largest
    ...
    real(kind=8) function pq_maxpri(a)
        type(pq), pointer :: a
    ...
    real(kind=8) function pq_replace_max(a, dis, idx)
        !
...
!    type(kdtree2_result), intent(in) :: e
        ...
        integer :: parent, child, N
        real(kind=8) :: prichild, prichildp1
...
module kdtree2_module
    ...
    type :: tree_node
        ...
        real(kind=8) :: cut_val_left, cut_val_right
    ...
    type :: kdtree2
        ...
        real(kind=8), pointer :: the_data(:, :) => null()
        ...
        real(kind=8), pointer :: rearranged_data(:, :) => null()
    ...
    type :: tree_search_record
        ...
        logical :: overflow
        real(kind=8), pointer :: qv(:)  ! query vector
        type(kdtree2_result), pointer :: results(:) ! results
        type(pq) :: pq
        real(kind=8), pointer :: data(:, :)  ! temp pointer to data
        ...
        if ((u - l) <= bucket_size) then
            ...
            if (.false.) then
                ...
                if (.true.) then
                    ...
                    average = sum(tp%the_data(c, tp%ind(l:u))) / real(u - l + 1, kind=8)
                else
...
!         res%cut_val = tp%the_data(c,tp%ind(m))
    ...
    subroutine spread_in_coordinate(tp, c, l, u, interv)
        ...
        ! ..
        ! .. Local Scalars ..
        real(kind=8) :: last, lmax, lmin, t, smin, smax
        integer :: i, ulocal
        ! ..
        ! .. Local Arrays ..
    ...
    subroutine kdtree2_n_nearest(tp, qv, nn, results)
        ...
        type(kdtree2), pointer :: tp
        real(kind=8), target, intent(In) :: qv(:)
...
!    deallocate(sr%pqp)
    ...
    subroutine kdtree2_n_nearest_around_point(tp, idxin, correltime, nn, results)
        ...
        sr%ballsize = huge(1.0)       ! the largest real(kind=8) number
        sr%centeridx = idxin
    ...
    recursive subroutine search(node)
        ...
        integer :: cut_dim, i
        ! ..
        real(kind=8) :: qval, dis
        real(kind=8) :: ballsize
        real(kind=8), pointer :: qv(:)
...
!          dis=extra**2
    ...
    real(kind=8) function dis2_from_bnd(x, amin, amax) result(res)
        real(kind=8), intent(in) :: x, amin, amax
    ...
    subroutine process_terminal_node(node)
        ...
        real(kind=8) :: ballsize, sd, newpri
        logical :: rearrange
        type(pq), pointer :: pqp
        !
    ...
    subroutine kdtree2_n_nearest_brute_force(tp, qv, nn, results)
        ...
        integer :: i, j, k
        real(kind=8), allocatable :: all_distances(:)
        ! ..
    ...
    subroutine heapsort(a, ind, n)
        ...
        !
        !
        real(kind=8) :: value   ! temporary for a value from a()
...
!       real(kind=8), Dimension (:,:), Pointer :: the_data
!       Integer :: dim, n
...
!       Private
!       real(kind=8) :: bsd
!       real(kind=8), Dimension (:), Pointer :: qv
!       Integer, Dimension (:), Pointer :: il
!       real(kind=8), Dimension (:), Pointer :: dsl
!       Integer :: nbst, nfound
...
!       ! ..
!       ! .. Local Scalars ..
!       real(kind=8) :: last, lmax, lmin, max, min, t
!       Integer :: i
!       ! ..
!       ! .. Local Arrays ..
!       real(kind=8), Pointer :: v(:,:)
...
!     ! ..
!     ! .. Array Arguments ..
!     real(kind=8), Target :: distances(n)
!     real(kind=8), Target, Intent (In) :: qv(:)
...
!     ! ..
!     ! .. Local Arrays ..
!     real(kind=8), Allocatable, Target :: qv(:)
!     ! ..
!     ! ..
!     Allocate (qv(tp%dim))
